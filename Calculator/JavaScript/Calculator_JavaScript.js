...
function Input_Decimal(dot) {
  ...
  if (!Calculator.Display_Value.includes(dot)) {
    //we are saying that if the display_value does not contain a decimal point, add a decimal point
...
function Handle_Operator(Next_Operator) {
  ...
  //when an operator key is pressed, we convert the current number displayed on the screen to a number and then store the result in Calculator.First_Operand if doesnt already exist
  const Value_of_Input = parseFloat(Display_Value);
  //check if an operator already exists and if Wait_Second_Operand is true, then updatres the operator and exits from the function
